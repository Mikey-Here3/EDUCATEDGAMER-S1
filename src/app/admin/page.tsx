...
export default async function AdminDashboardPage() {
  ...
  try {
    const [tRes, aRes, pRes] = await Promise.all([
      ...
      sql`SELECT COUNT(*)::int as count FROM teams WHERE status = 'approved';`,
      sql`SELECT COUNT(*)::int as count FROM teams WHERE status = 'pending';`,
    ])
    totalTeams = tRes[0]?.count || 0
    approvedTeams = aRes[0]?.count || 0
    pendingTeams = pRes[0]?.count || 0
  } catch (err) {
    console.error('Neon admin dashboard query error:', err)
  }
  ...
  const cards = [
    ...
    { label: 'Slots Remaining', value: Math.max(0, 12 - approvedTeams), icon: Trophy, color: 'text-red-400', bg: 'bg-red-500/10 border-red-500/20' },
  ]
  ...
  return (
    <div className="p-6 space-y-8 text-white">
      <div>
        <h1 className="text-3xl font-black tracking-tight text-white font-heading">Neon Admin Dashboard</h1>
        <p className="text-gray-500 text-sm mt-1">Educated Gamer Tournament Control Panel (Powered by Neon Postgres)</p>
      </div>
